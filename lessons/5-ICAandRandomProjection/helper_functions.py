...
def plot_rp(X, projected_data, n_components):
    ...
    plt.hexbin(dists, projected_dists, gridsize=1000, cmap=plt.cm.Blues, extent=[min_dist, max_dist, min_dist, max_dist])
    plt.xlabel("Pairwise squared distances in original space")
    plt.ylabel("Pairwise squared distances in projected space")
    plt.title("Pairwise distances distribution for {} components".format(n_components))
    cb = plt.colorbar()
    cb.set_label('Sample pairs counts')

    ratio = projected_dists / dists
    plt.figure()
    plt.hist(ratio, bins=50, range=(0., 2.), edgecolor='k')
    plt.xlabel("Squared distances ratio: projected / original")
    plt.ylabel("Distribution of samples pairs")
    plt.title("Histogram of pairwise distance ratio for {} components".format(n_components))
